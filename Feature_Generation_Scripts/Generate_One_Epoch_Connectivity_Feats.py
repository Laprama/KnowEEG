...
# Specify parameters for the script __________________________________
metrics = [ 'coh', 'imcoh' ,'dpli' ] # One epoch metrics that will be calculated and saved
...
for metric in metrics:
    ...
    for sample in X_data : 
        ...
        sample = sample.reshape(1, n_electrodes, n_sequence_length)
    ...
    connectivity_df = connectivity_df.reset_index(drop = True) # reset the index of the dataframe before saving it
    
    #Save the features________________
    file_name = 'connectivity_one_epoch_' + metric + '.pkl' 
